...
class ensemblemodel(nn.Module):
    def __init__(self,available_actions_count) -> None:
        ...
        self.initial_layerd = nn.Sequential(nn.Conv2d(1,3,3,1,1), nn.ReLU())
...
class DQNAgent:
    def __init__(
        ...
        if load_model:
            ...
            self.q_net = ensemblemodel(action_size).to(DEVICE)
            self.target_net = ensemblemodel(action_size).to(DEVICE)
